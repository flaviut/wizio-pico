...
def add_ops(env):
    ...
    if (
        "PICO_STDIO_USB" in env.get("CPPDEFINES")
...
def add_tinyusb(env):
    ...
    for define in env.get("CPPDEFINES"):
        if "_USB" in define:
            env.Append(
                CPPDEFINES=["CFG_TUSB_MCU=OPT_MCU_RP2040", "CFG_TUSB_OS=OPT_OS_PICO"],
                CPPPATH=[USB_DIR],
            )
            env.BuildSources(
                OBJ_DIR,
                USB_DIR,
                src_filter=["+<*>", "-<host>", "-<device>", "-<class>"],
            )
            print("  * TINYUSB")
            break

    if "PICO_STDIO_USB" in env.get("CPPDEFINES") or "LIB_PICO_STDIO_USB" in env.get(
        "CPPDEFINES"
    ):
        env.BuildSources(
            OBJ_DIR, USB_DIR, src_filter=["-<*>", "+<device>", "+<class/cdc>"]
        )
        return
